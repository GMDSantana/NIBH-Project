...
int SalvaResultados(stationData stat, double P)
{
    struct tm tm1 = { 0 };
    int horas, minutos;
    int isEmpty;
    ...
    /* A variavel "data" contem apenas o dia do ano (0-365), mas o SWMM requer dia, mes e ano.
     * Entao a partir de "dia do ano" (data) + "ano" + time.h descobre dia, mes e ano. */
    tm1.tm_year = 2000 - 1900 + stat.ano % 4;
    tm1.tm_mday = stat.data;
    tm1.tm_hour = 12;
    mktime(&tm1);
    tm1.tm_year = stat.ano;
    tm1.tm_mon++;

    // Tambem eh necessario formatar o tempo, que vem em hhmm, para hh\tmm
    minutos = stat.tempo%100;
    horas = stat.tempo/100;

    if( (outFile = fopen("PrecipitacaoPrevista.dat", "a")) == NULL)
    {
    ...
    fprintf(outFile, "1\t%d\t%d\t%d\t%d\t%d\t%f\n", tm1.tm_year, tm1.tm_mon, tm1.tm_mday, horas, minutos, P);
    fclose(outFile);

    return 1;
    // Abre (ou cria se não houver) arquivo de saida em ASCII com nome padrao: outPrec.dat
...
//double FuncaoGeorgakakos(int ano, int data, int tempo, float temperatura, float umidade, float pressao, double pobs,
//                       float topoDosEcos, float VIL, int *iteracao)
double FuncaoGeorgakakos(stationData stat, double pobs, int *iteracao)
{
...
double precipitacao(stationData *stat, double *pobs, int *iteracao)
{
    double P;

    /* Calculos de previsao */
    P = FuncaoGeorgakakos(*stat, *pobs, iteracao);
    //pobs: regressao e etc

    /* Gera arquivo de saida para o SWMM */
    SalvaResultados(*stat, P);
