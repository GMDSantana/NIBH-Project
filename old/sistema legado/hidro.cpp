/*APÊNDICE C – CÓDIGOS EM C++ BUILDER


Modelo hidrometeorológico adaptado */
//---------------------------------------------------------------------------
...
// for(int linha_m=1;linha_m<2;linha_m++) //total de linhas
    {
        ...
        {
            vet_resultados[18][linha_m] = vet_resultados[17][linha_m] / (StrToFloat(densidade-
                                          >Text)*StrToFloat(area->Text));
...
void carrega_vetor_altura(float altura,int total_intervalo)
{
...
void limpa_vetor_altura(int altura, int total_intervalo)
{
...
float calculaMedia(int numIntervalos, float *elementos)
{
    float somatorio = 0.0;
...
float calculaVariancia(int numIntervalos, float media, float *elementos)
{
    float variancia = 0.0;
...
float calculaCovariancia(int numIntervalos, float media, float *elementos)
{
    float media1 = 0.0,
          media2 = 0.0,
          somatorio = 0.0,
...
float calculaAutoCorrelacao(float covariancia, float variancia)
{
...
void TFPrincipal::Regressao(int e_ano,int data,int tempo,float temp_externa, float umidade_ar, float pressao_atm, int linha,int total_linha)
{
    ...
    if ((FPrincipal->VIL[FPrincipal->linha] != -1) || (FPrincipal->TopoDosEcos[FPrincipal->linha] != -1))
    {
        ...
        covarTopoDosEcos = calculaCovariancia(FPrincipal->linha, mediaTopoDosEcos, FPrincipal-
                                              >TopoDosEcos);
...
//------------------------------------------------------------------------------
//void TFPrincipal::converteDados(int data,int tempo, float temp_externa,float umidade_ar, float pressao_atm, int linha, float VIL, float TopoDosEcos) //n por linha
//Usando e1 = 0.007925, e2 = 44028.152344, e3 = 0.231883, e4 = 0.000041
//double* TFPrincipal::converteDados(float e1, float e2, float e3, float e4) //n por linha double* TFPrincipal::converteDados(float e1, float e2, float e3, float e4) //n por linha
{
    int i;
    float aux, aux_tm, aux_l, aux_w, tolerancia, tolerancia_v, v2,v1,res, pressao;

    double *retorno = new double[2];
    fprintf(streamLog, "e1 = %f, e2 = %f, e3 = %f, e4 = %f", e1, e2, e3, e4);
    try
    {
        for (i = 1; i <= FPrincipal->linha; i++)
        {
//fprintf(streamLog, "- Linha %d", i);
///dia[i]=data;
///hora[i]=tempo;
...
//pt1[i] = ((StrToFloat(e2->Text)-StrToFloat(pl->Text) )/(1+StrToFloat(e3->Text)*v1))+StrToFloat(pl->Text);
                pt1[i] = ((e2 - StrToFloat(pl->Text) )/(1+e3*v1))+StrToFloat(pl->Text);
// pt1[i]=1.1*ps[i];
...
// subst valor de pt na eq 16 e encontra tslinha
                ...
                fprintf(streamLog,"Parametros tslinha: To[%d] = %f, po[%d] = %f, aux = %f", i, To[i], i, po[i], aux); tslinha[i] = (To[i]/pow(po[i],0.286))* pow(aux,0.286); //encontra tslinha
                fprintf(streamLog,"tslinha[%d] = %f", i, tslinha[i]);
// subst Tm e Ts' na eq 14 e encontra deltat
                DELTAT[i]= Tm[i] - tslinha[i];
// vai pra 13 e encontra v2
                fprintf(streamLog,"Parametros v1: CP*DELTAT[%d] = %f", i, CP*DELTAT[i]);
                v1 = e1*pow((CP*DELTAT[i]),0.5);//eq 13
                fprintf(streamLog,"v1 = %f", v1);
//calcula p novamente
                pt[i] = (e2-StrToFloat(pl->Text) )/(1+e3*v1)+StrToFloat(pl->Text);
...
//c eq 19 estudo de caso
            fprintf(streamLog,"Parametros c[%d]: v[%d] = %f, K = %f", i, i, v[i], K);
            c[i]=1/e4*pow(v[i],K);
            fprintf(streamLog,"c[%d] = %f", i, c[i]);
//ECONTRA TT: apaguei.
//tetae eq. 16
//erro*********
...
//Razao de mistura ws(Tt,pt)
            fprintf(streamLog,"Parametros wst[%d]: Tt[%d]-223.15 = %f", i, i, Tt[i]-223.15);
            wst[i]=0.622*(A1*(pow((Tt[i]-223.15),3.5)))/pt[i];
            fprintf(streamLog,"wst[%d] = %f", i, wst[i]);
...
//Ob e Ot em funcao da massa, ou seja, nao esta multiplicado por X[i]
            ...
            fprintf(streamLog,"F1 = %f", F1);
            F2 = pow(Nv[i],3)*0.0416667;
            fprintf(streamLog,"F2 = %f", F2);
            F3 = (F1+F2)/exp(Nv[i]);
            if (Zc[i] == 0)
                Ob1[i] = 0;
            else
            ...
            fprintf(streamLog,"Ot[%d] = %f", i, Ot[i]);
...
//double pa = (X[i+1]/(DELTA*Zc[i]))*vp[i];
                    ...
                    fprintf(streamLog,"pb = %f", pb);
                    double pc = (pow(Nd[i],3)*0.125);
                    fprintf(streamLog,"pc = %f", pc);
                    double pd = exp(Nd[i]);
                    double pe = (pb+pc)/pd;
                    P[i] = pa*pe;
                }
    ...
    retorno[0] = (double)FPrincipal->CalculaDeltaVol(linha, FPrincipal->P, FPrincipal->Pobs);
    retorno[1] = (double)FPrincipal->CalculaCorrel(linha, FPrincipal->P, FPrincipal->Pobs);
    return retorno;
}
...
// Parametrizacao para encontrar v e pt.
    ...
    {
// subst v1 na eq 18, encontra pt
        pt1[i] = ((StrToFloat(e2->Text)-StrToFloat(pl->Text) )/(1+StrToFloat(e3->Text)*v1))+StrToFloat(pl-
                 >Text);
// pt1[i]=1.1*ps[i];
// subst pt1 na eq 15 encontra p'
        plinha[i] = ps[i] - ((0.25)*(ps[i]-pt1[i]));
...
//calcula p novamente
        pt[i] = ((StrToFloat(e2->Text)-StrToFloat(pl->Text) )/(1+StrToFloat(e3->Text)*v1))+StrToFloat(pl-
                >Text);
// compara os dois valores de p
        res = pt1[i]-pt[i];
        tolerancia_v = pt1[i]*2/100;
...
// compara os dois valores de v
// v2 = ((StrToFloat(e2->Text)-StrToFloat(pl->Text) )/(1+StrToFloat(e3->Text)*v1))+StrToFloat(pl-
        >Text);
// res = v1-v2;
// tolerancia_v = v1*2/100;
// v1=v2;
    ...
    try
    {
        ...
        while (linha <= FPrincipal->SG_final->RowCount)
        {
            for (int j=0; j<FPrincipal->SG_final->ColCount; j++)
                ...
                Sheet.OlePropertyGet("Cells").OlePropertyGet("Item",linha+1,j+1).OlePropertySet("Value",FPrincipal-
                        >SG_final->Cells[j][linha].c_str());
            linha++;
        }
